    ...
    return (
        <View style={styles.pageContainer}>
            {[...Array(General.WELCOME_CONTENTS.length).keys()].map((_, i) => 
                    i === index ? (<View style={pageStyle(true)} key={i}></View>)
                                    :
                                    (<View style={pageStyle(false)} key={i}></View>),
            )}
        </View>
    )
} // 페이징 처리시 컨테이너 값 지정 (*특히 인덱스 처리시 함수로 처리해야함. 변수 처리시 페이지스타일 안먹힘)

const WelcomeScreen = ({navigation}) => {
    const [welcomeListIndex, setWelcomeListIndex] = useState(0); //useState 처리로 (훅)
    ...
    const navigate = () => {
        StorageEvent.setFirstTimeUse().then(() => {
  ...
  return (
    <View style={styles.container}>
        ...
        <Separator height={Display.setHeight(8)} />
        {welcomeListIndex === 2 ? (<TouchableOpacity style={styles.gettingStartedButton} 
                                                     activeOpacity={0.8} onPress={() => navigation.navigate('Signin')}>
                                        <Text style={styles.gettingStartedButtonText}>배고프면 빨리 눌러</Text>
                                    </TouchableOpacity>)
                                : 
